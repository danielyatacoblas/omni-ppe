...
TITULO = "OMNI PPE"
LEMA = "De un vigilante mirando a un «puede entrar» por persona"
FRASE = ("Repartir por «el más cercano» le daría el casco de un operario al de al lado, y los dos saldrían aprobados llevándolo uno.")
...
def main() -> int:
    ...
    PASOS = [
        ("01", "video", "Entra", "La cámara de la puerta",
         f"0 €", f"en control de acceso"),
        ("02", "casco", "Detecta", "SH17 · 17 clases de EPP",
         f"{acierto}", f"mAP@50 · umbral 0.35"),
        ("03", "personas", "Reparte", "A quien más lo contiene",
         f"4", f"ítems por persona"),
        ("04", "visto", "Decide", "LISTO o NO LISTO",
         f"80 %", f"umbral para pasar"),
    ]
    ...
    n = len(med)
    for i, (et, val) in enumerate((("Pruebas", f"13 pasando"), ("Modelos medidos", f"{n}"), ("Corre en", f"CPU o GPU"), ("Datos simulados", f"ninguno"))):
        x = m + i * ((W - 2 * m) / 4)
